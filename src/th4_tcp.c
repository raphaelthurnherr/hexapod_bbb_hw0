...
// TRAITEMENT DE LA COMMANDE RECUE
			...
			printf("Réception commande TCP: %d\n",tcpData);

			tcpWriteString("Séquence: ",10);
			tcpWrite(tcpData);
...
/*						if(AI_enable){
			...
			default :   tcpWriteString("Séquence: INCONNUE",18);
						break;
			}
...
// FIN DE TRAITEMENT
		...
		if(tcpData==-1) printf("Client TCP déconnecté...");
