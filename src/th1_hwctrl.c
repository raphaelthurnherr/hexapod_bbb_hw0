...
// Affichage de la trame recue décodée ou non
...
unsigned char th1_LowBatteryCheck(void);	// Contrôle etat des batteries
...
void setMotorState(unsigned char motor, unsigned char state); 						// défini l'état ON/OFF du moteur
void setMotorsInterrupt(unsigned char Enable); // Active/désactive l'interruption lorsques les moteur sont en position
...
// Mise à jour de l'angle d'orientation
...
void *hwctrlTask (void * arg)
{
	...
	// Initialisation GPIO du système
	GpioSetup();
...
//	  pthread_mutex_lock (&my_mutex);
	  ...
	  // Fin de déplacement du servomoteur EYES, Démarre la mesure
...
//	pthread_mutex_unlock (&my_mutex);
  ...
  pin_low(8,GPIO_BUZZER);								// Désactive gpio buzzer
  iolib_free();											// Libère l'accès aux gpios
...
// Retourne La distance à un angle donné, moteur vitesse max !
void getUltrasonicDistance(unsigned char angle){
...
// ---------------------------------------------------------------------------
// Contrôle de la reception d'une trame valide
// ---------------------------------------------------------------------------
...
// READ BACK ULTRASON APRES ENVOIE DE COMMANDES
 	 	 case 0	:	updateMotorState();						// Echo commande moteur (ON/OFF)
					...
					break;
	 	 case 1	:	updateMotorAngle();						// Echo commande positionnement moteur pleine vitesse(0..90 degrés)
	 	 	 	 	motorsPositionValid=1;
					break;
	 	 case 2	:											// Echo commande positionnement moteur vitesses réduite(0..90 degrés)
					break;
...
// READ BACK ULTRASON APRES ENVOIE DE COMMANDES
	 	 case 10:	updateUltrasonicDistance();				// Echo demande de lecture du capteur ultrason
					break;									// +Datas: validité de la mesure(8bit bool), mesure en mm (16bit)
...
// READ BACK BOUSSOLE APRES ENVOIE DE COMMANDES
	 	 case 20:	updateCompassAngle();					// Echo demande de lecture du capteur ultrason
					break;									// +Datas: validité de la mesure(8bit bool), mesure en degrees avec une décimale (16bit)
		 case 21:											// Echo stop mesure sur boussole
					break;
		 case 22:											// Echo commande interruption boussole ON/OFF
					break;
		 case 23:											// Echo commande paramètrage interruptions dans une plage donnée Min(16bit)+Max(16bit)
					break;
	 	 case 25:											// Echo de commande de démarrage calibration du capteur
					break;									// (Fonctionne en tournant à 360deg sur axes XYZ l'appareil)
...
// READ BACK INFRAROUGES APRES ENVOIE DE COMMANDES
		 ...
		 case 32:											// Echo commande interruption Infrarouge ON/OFF (sur changement d'état)
					break;
...
// RECUE SI INTERRUPTIONS ACTIVEES
		 ...
		 case 120:	updateCompassAngle();					// Interruption boussole + valeur de mesure
					break;
		 case 130:	updateIRstate();						// Interruption capteurs IR + états
					break;
...
void GpioSetup(void){
	...
	iolib_setdir(8,GPIO_BATT_MOTOR, BBBIO_DIR_IN);		// P8.x en entrée pour warning batterie
	iolib_setdir(8,GPIO_BATT_BRAIN, BBBIO_DIR_IN);		// P8.x en entrée pour warning batterie
...
void buzzerCtrl(unsigned char bipCount){
	...
	if(bipCount>0){
		nbBipToDo=bipCount;				// Bip demandé par utilisateur
	}
}
...
// ----------------------------------------------
// Définit un angle sur un servomoteur
// ----------------------------------------------
void setMotorAngle(unsigned char motor, unsigned char angle, unsigned char speed){
	...
	while(flag_uartOutBusy);
		...
		flag_uartOutBusy=0;
		usleep(100000);	// Sleep pour vitesse MAX Mecanique. (sous-evalué...)
}


// ----------------------------------------------
// Définit l'état du servomoteur
// ----------------------------------------------
...
// ----------------------------------------------
// Active/désactive l'interruption lorsques les moteur sont en position
// ----------------------------------------------
...
// ----------------------------------------------
// Active/désactive les interruptions d'evenement de capteurs IR
// ----------------------------------------------
void SetIRinterrupts(unsigned char Enable){
	while(flag_uartOutBusy);
	flag_uartOutBusy=1;
...
// ----------------------------------------------
// Envoie une trame complète prédéfinie
// ----------------------------------------------
void sendUartFrame(unsigned char *buffToSend, unsigned char nbByte){
