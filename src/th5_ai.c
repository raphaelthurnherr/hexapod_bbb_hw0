...
//	  pthread_mutex_lock (&my_mutex);
	  ...
	  // Démarrage de la séquence en autonomie
	  if(AIenable){
		  if(!compassIsCalibrate){
				  ...
				  hexapodCalibrateCompass();		// Démarre la calibration boussole en mouvement
			  ...
			  if (!USMeasureStarted){
				  StartUltrasonicMeasure();		// Démarre une mesure permanente de distance;
