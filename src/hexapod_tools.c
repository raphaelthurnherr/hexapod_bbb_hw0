...
// Contrôle de l'état des batteries affichage alarmes
...
// ***************************************************************************
// Contrôle de l'état des batteries affichage alarmes
// Entrée: Affichage du status de batteries ON/OFF
// Sortie: Retour d'état warning [0:NO WARNING 1: BATT1, 2: BATT2, 3; BATT+2]
// ***************************************************************************
...
// Scan la distance d'un angle donné à un autre avec un pas et stock les valeurs à une adresse définie
// Entrée: fromAngle, Angle de départ
// Entrée: toAngle, Angle de d'arrivée
// Entrée: angleStep, nombre de degres entre les scans
// Entrée: Target, addresse pour enregistrement des valeurs [0] si pas d'enregistrement];
// ***************************************************************************
void ScanZone(unsigned char fromAngle, unsigned char toAngle, unsigned char angleStep, unsigned char *Target){
...
// Entrée: AngleScanDept, Tableau de valeur de distance pour chaque angle (0..90)
// Entrée: Résolution angulaire à traiter [degres]
// ***************************************************************************
...
// Entrée: Résolution angulaire à traiter [degres]
// Entrée: filtre à appliquer (0..2) 0 = pas de filtre
// ***************************************************************************
...
void hexapodCalibrateCompass(void){
	...
	printf("VITESSE 2 \n");
	// Défini la vitesse (moyenne)
	setMotionSpeed(2);
	usleep(1000000);

	// Séquence activation des moteurs
	printf("ACTIVATION MOTEUR \n");
	setMotion(0);
	usleep(1000000);

	printf("CALIBRATION BOUSSOLE \n");
	// Démarre la calibration de la boussole
	CompassCalibrate();
	usleep(1000000);

	printf("ROTATION... \n");

	// Séquence rotation gauche
	setMotion(5);
	usleep(1000000);
